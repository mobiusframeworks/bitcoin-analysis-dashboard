#!/usr/bin/env python3
"""
Run ML Model Selection Pipeline - PREDICTING RETURNS

This version:
1. Predicts RETURNS (percentage changes) instead of absolute prices
2. Uses proper time series features (lagged)
3. Applies comprehensive feature selection
4. Separates FRED vs Technical features
5. Removes redundant/correlated features
6. Comprehensive overfitting checks
"""
...
print("="*80)
print("BITCOIN RETURN PREDICTION - WITH FEATURE SELECTION")
print("="*80)
print()
print("Target: 20-day forward returns (percentage change)")
print("This is a more realistic prediction task than absolute prices.")
print()

# Configuration
DATA_PATH = "datasets/Bitcoin/bitcoin data csvs/btc-ohlc.csv"
TARGET_LAG_DAYS = 20  # Predict 20 days ahead
TASK_TYPE = "REGRESSION"
TIME_COL = "d"
IS_TIMESERIES = True
OUTPUT_DIR = "ml_pipeline/bitcoin_results_returns"
...
# TARGET: 20-day forward returns (percentage change)
...
# Remove rows where target is NaN (from forward-looking calculation)
valid_mask = ~target.isna()
df_features = df_features[valid_mask].copy()
target = target[valid_mask].copy()

# Remove any remaining NaN in features
...
print(f"  Mean: {target.mean():.4f} ({target.mean()*100:.2f}%)")
print(f"  Std: {target.std():.4f} ({target.std()*100:.2f}%)")
print(f"  Min: {target.min():.4f} ({target.min()*100:.2f}%)")
print(f"  Max: {target.max():.4f} ({target.max()*100:.2f}%)")
print()
...
selected_features, selection_report = comprehensive_feature_selection(
    ...
    correlation_threshold=0.90,  # More aggressive: Remove features with >90% correlation
    cross_category_threshold=0.85,  # More aggressive: Remove FRED/Technical conflicts >85%
    use_mutual_info=True,
    top_k=10  # Keep only top 10 features (more aggressive)
)
...
for step in selection_report['steps']:
    ...
    if 'removed_features' in step and step['removed_features']:
        print(f"  Removed features: {', '.join(step['removed_features'][:5])}")
...
prepared_data_path = Path(OUTPUT_DIR) / "bitcoin_prepared_returns.csv"
...
df_features_selected['close'] = df.loc[df_features.index, 'close'].values  # Keep for reference
...
# For returns prediction, adjust threshold (returns are harder to predict)
overfitting_report = comprehensive_overfitting_check(
    ...
    threshold=0.15  # More lenient for returns
)
...
# Calculate confidence intervals based on return prediction errors
test_pred = best_model.predict(X_test)
test_errors = y_test - test_pred
error_std = test_errors.std()

# Convert return error to price error
